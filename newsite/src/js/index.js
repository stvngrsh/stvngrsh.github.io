...
function handleScroll() {
  ...
  if (blueOffset >= 200) {
    ...
    let xP2 = blueOffset;
    clipPath =
      "polygon(0 " + xP2 + "px, 100% " + xP1 + "px, 100% 100%, 0 100%)";
    $("#logo-bottom").removeClass("red");
  ...
  const projTop = $("#projects").offset().top;
  const resumeTop = $("#resume").offset().top;

  const navChange = navTop - scroll;
  const projChange = projTop - scroll;
  const resumeChange = resumeTop - scroll;

  if (resumeChange <= 1) {
    $(".nav-item").removeClass("active");
    $("#nav-resume").addClass("active");
  } else if (projChange <= 1) {
    $(".nav-item").removeClass("active");
